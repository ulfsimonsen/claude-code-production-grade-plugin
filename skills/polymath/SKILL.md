...
- **Grep** — find specific patterns, symbols, imports, business logic markers across the codebase
- **Read** — deep-read specific files identified as important (use offset/limit for large files)
...
### Efficiency
- Always parallel: when onboarding a repo, issue Glob + Grep + Read simultaneously
- Always parallel: when researching, issue multiple WebSearch calls for different angles
- Always Grep before full Read — don't read 500-line files to find one function
